...
class RNNModel:
    ...
    def __init__(self, buckets_dict, mode='training',params={}):
        ...
        if mode == 'training':
            ...
            for bucket_num, bucket in buckets_dict.items():
                ...
                self.token_lookup_sequences_placeholder_list.append(tf.placeholder("int32",
                    [None, max_sentence_length], name="token_lookup_sequences"+str(bucket_num))) # token_lookup_sequences
                ...
                self.token_output_sequences_decoder_placeholder_list.append(tf.placeholder("int32",
                    [None, max_sentence_length], name="token_output_sequences_decoder_placeholder"+str(bucket_num)))
                self.token_lookup_sequences_decoder_placeholder_list.append(tf.placeholder("int32",
                    [None, max_sentence_length], name="token_lookup_sequences_decoder_placeholder"+str(bucket_num))) # token_lookup_sequences

                self.token_output_sequences_decoder_inpmatch_placeholder_list.append(tf.placeholder("float32",
    ...
    def getEncoderModel(self, config, bucket_num=-1, mode='training', reuse=False):
        ...
        if mode == 'training':
            ...
            self.token_lookup_sequences_placeholder_inference = tf.placeholder("int32",
                [None, max_sentence_length], name="token_lookup_sequences") # token_lookup_sequences
            token_lookup_sequences_placeholder = self.token_lookup_sequences_placeholder_inference
        ...
        if reuse:
            ...
            self.encoder_token_emb_mat = token_emb_mat = self.initEmbeddings('emb_encoder', token_vocab_size,
                        embeddings_dim, reuse=reuse, pretrained_embeddings=pretrained_embeddings,
        ...
        # run lstm
        outputs_tensor = self.encoderRNN(inp, lstm_cell_size, config['batch_size'], max_sentence_length,
                                         reuse, mode=mode)
