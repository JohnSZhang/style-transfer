...
class Solver:
    ...
    def runInference(self, config, encoder_inputs, decoder_ground_truth_outputs, reverse_vocab, sess,
                     print_all=True, print_gt=True):  # sampling

        model_obj = self.model_obj

        feed_dct = {
            model_obj.token_lookup_sequences_placeholder_inference: encoder_inputs
        }

        decoder_outputs_inference, encoder_outputs, alpha_inference = \
            np.array(sess.run([self.decoder_outputs_inference, self.encoder_outputs,
                               self.alpha_inference], feed_dict=feed_dct))  # timesteps, N
    ...
    def solveAll(self, config, encoder_inputs, decoder_ground_truth_outputs, reverse_vocab, sess=None,
        ...
        for i in range(num_batches):
            ...
            if inference_type == "greedy":
                decoder_outputs_inference_cur, alpha_cur = self.runInference(config, encoder_inputs_cur,\
                                                                             decoder_ground_truth_cur, reverse_vocab,\
                                                                             sess=sess, print_all=print_progress,
                                                                             print_gt=print_progress)
